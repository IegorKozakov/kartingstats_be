...
function processInitMessage(message) {
    const grid = message.filter((item) => item.includes("grid|"));
    const table = grid[0].split("||")[1];
    if (!table) {
        console.error("Can not load table");
        return null;
    }
    const dom = (0, node_html_parser_1.parse)(table);
    ...
    const rkCellNumber = dom
        ...
        ?.getAttribute("data-id");
    const onTrackNumber = dom.querySelector("td[data-type='otr']")?.getAttribute("data-id") ?? null;
    const teamLaps = dom.querySelector("td[data-type='tlp']")?.getAttribute("data-id") ?? null;
    rows.forEach((row) => {
        let dataId = row.getAttribute("data-id");
        let isPit = row.querySelector(`td[data-id="${dataId}c2"]`)?.getAttribute("class") ??
            "";
        let teamName = row.querySelector(`td[data-id="${dataId}${driverCellNumber}"]`)
            ?.textContent ?? "";
        let stint = row.querySelector(`td[data-id="${dataId}${onTrackNumber}"]`)
            ?.textContent ?? "0";
        let kart = row.querySelector(`div[data-id="${dataId}${kartCellNumber}"]`)
            ?.textContent ?? "";
        let position = row.querySelector(`p[data-id="${dataId}${rkCellNumber}"]`)?.textContent ??
            "";
        let lapTime = row.querySelector(`td[data-id="${dataId}${lastLapCellNumber}"]`)
            ?.textContent ?? "";
        let lapsAmount = row.querySelector(`td[data-id="${dataId}${teamLaps}"]`)?.textContent ??
            "";
        if (!kart) {
...
function convertToMSFromHours(time) {
    if (time.length < 2) {
        return time;
    }
    let splittedMilisec = time.split(":");
    ...
    return (parseInt(splittedMilisec[0]) * 60 * 60000 +
        parseInt(splittedMilisec[1]) * 60000);
}
//# sourceMappingURL=apex-message-processor.js.map